			       ...
			       GBytes **used_reldata)
{
	...
	if (as_release_list_get_kind (releases) == AS_RELEASE_LIST_KIND_EXTERNAL) {
		g_autofree gchar *relmd_uri = NULL;

		as_release_list_clear (releases);
		if (allow_net && as_release_list_get_url (releases) != NULL) {
			/* get the release data from a network location */
			const gchar *releases_url = as_release_list_get_url (releases);
	...
	if (as_component_get_kind (cpt) != AS_COMPONENT_KIND_OPERATING_SYSTEM) {
		if (as_release_list_len (releases) > MAX_RELEASE_INFO_COUNT)
			as_release_list_set_size (releases, MAX_RELEASE_INFO_COUNT);
	}
