...
test_yaml_basic (void)
{
	...
	path = g_build_filename (datadir, "dep11-0.16.yml", NULL);
...
static const gchar *yamldata_releases_field =
    ...
    "acbad92084387c73f696f00368ea106ebb4\n"
    "    size: {}\n"
    "  - type: binary\n"
...
test_yaml_read_releases (void)
{
	...
	g_assert_cmpint (as_release_list_len (as_component_get_releases_plain (cpt)), ==, 2);

	rel = as_release_list_index (as_component_get_releases_plain (cpt), 0);
...
test_yaml_rw_utf8 (void)
{
	static const gchar
	    *yamldata = "Type: generic\n"
			...
			"supports FTP using its own FTP API and\n"
			"    various other protocols like SMB, SFTP, NFS, HTTP, and file I/O using "
			"third party APIs. It includes many\n"
			"    advanced features such as recursive directory up/download, browsing "
			"FTP servers while transferring files,\n"
			"    FTP resuming and queueing, browsing the LAN for Windows shares, and "
			"more. Multiple connections can open\n"
			"    at a time in a Mozilla-style tabbed browsing environment.</p>\n"
			"  uk: |-\n"
			"    <p>Jftp — це графічний клієнт передачі даних мережею. Підтримка FTP "
			"реалізована за допомогою внутрішнього\n"
			"    інтерфейсу, а підтримку протоколів SMB, SFTP, NFS, HTTP та роботу з "
			"файлами — за допомогою зовнішніх\n"
			"    інтерфейсів. Він має багато гарних можливостей, наприклад, рекурсивне "
			"відправлення/завантаження, роботу\n"
			"    з FTP-сервером під час передачі файлів, продовження та створення черг "
			"завантажень з FTP, перегляд локальної\n"
			"    мережі в пошуках спільних тек Windows, тощо. Може створювати по "
			"декілька з&apos;єднань одразу за допомогою\n"
			"    подібного до Mozilla інтерфейсу з вкладками.</p>\n";
