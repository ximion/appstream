		...
		for (guint j = 0; issue_cids[j] != NULL; j++) {
			...
			for (guint k = 0; k < hints->len; k++) {
				...
				if (mode == ASC_REPORT_MODE_FULL) {
					...
					g_autofree gchar *text_wrap = NULL;

					text = asc_hint_format_explanation_plain (hint);
					text_wrap = ascli_format_long_output (text, 100, 5);
					g_string_append (report, text_wrap);
					g_string_append_c (report, '\n');
				}
			}
			if (!entry_added)
