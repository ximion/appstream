...
as_cache_builder_scan_apt (AsCacheBuilder *builder, gboolean force, GError **error)
{
	...
	/* We can't do anything here if we can not write to these directories.
	 * We don't emit an error here, since this issue is a completely expected result
	 * when the cache is generated by a non-root user to a custom directory.
	 */
	if (!as_utils_is_writable (appstream_yml_target)) {
		g_debug ("Unable to write to '%s': Can add AppStream data from APT to the pool.", appstream_yml_target);
		return;
	}

	if (!as_utils_is_writable (appstream_icons_target)) {
		g_debug ("Unable to write to '%s': Can add AppStream data from APT to the pool.", appstream_icons_target);
		return;
	}
